 ...
 */
@SdkInternalApi
class S3FileAsyncResponseHandler<ResponseT> implements AsyncResponseHandler<ResponseT, ResponseT> {
    ...
    S3FileAsyncResponseHandler(Path path, Subscriber<ByteBuffer> subscriber) {
        this.path = path;
        this.subscriber = subscriber;
    }

    S3FileAsyncResponseHandler(Path path) {
        this.path = path;
        this.subscriber = new FileSubscriber();
    }
            ...
            fileChannel.write(byteBuffer, position.get(), byteBuffer, new CompletionHandler<Integer, ByteBuffer>() {
                ...
                public void completed(Integer result, ByteBuffer attachment) {
                    if (result > 0) {
                        ...
                        synchronized (S3FileAsyncResponseHandler.FileSubscriber.this) {
                            if (closeOnLastWrite) {
                                invokeSafely(fileChannel::close);
                            } else {
